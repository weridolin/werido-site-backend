...
class RbacConfig(AppConfig):
    ...
    def ready(self) -> None:
        ...
        #             add_permission(orm=ModelOperation,op_name=f"view",op_model_name=name,description=f"对表:{name}查询的权限")
        #             add_permission(orm=ModelOperation,op_name=f"delete",op_model_name=name,description=f"对表:{name}删除的权限")
        #             add_permission(orm=ModelOperation,op_name=f"post",op_model_name=name,description=f"对表:{name}增加的权限")
        #             add_permission(orm=ModelOperation,op_name=f"update",op_model_name=name,description=f"对表:{name}修改的权限")                    


        # ## 初始化角色
        # print("初始化角色信息...")
        # super_role,_ =Role.objects.update_or_create(role_name="超级管理员")
        # Role.objects.update_or_create(role_name="游客")
